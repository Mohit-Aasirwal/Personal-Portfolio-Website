...
const Modal = ({ name, image, link, description }: props) => {
  ...
  return (
    <div className="">
      <div
        className="flex bg-white w-28 h-28 rounded-xl shadow-inner shadow-black drop-shadow-xl hover:-translate-y-3 ease-linear duration-150"
        onClick={handleOpen}
      >
        <Image
          src={image}
        ...
        <div className="md:w-2/3 w-[90%] md:rounded-3xl select-none shadow-inner fixed shadow-black text-black top-1/4 left-5 scroll-smooth md:top-52 md:left-60 overflow-scroll md:overflow-hidden z-50 bg-white h-2/3">
          <div className="flex flex-col justify-start ">
            ...
            <div className="flex md:flex-row flex-col justify-between md:ml-10 mx-5 md:mx-0">
              <div className="md:w-1/2 h-fit flex py-5 text-left flex-col justify-start items-start space-y-10 order-2 md:order-1">
              ...
              <div className="md:w-1/2 md:pb-10 md:px-10 order-1 md:order-2 py-5 md:py-0 ">
                <Image
                  src={image}
                  alt="images"
                  width={1000}
